...
CSR_OUTPUT_FOLDER = os.path.join(OUTPUT_FOLDER, "test_csr_output_files")
...
# Items needed by test_build_device_csr
DEVICE_CSR_FILENAME = os.path.join(CSR_OUTPUT_FOLDER, "device.csr")
CSR_DUMMY_COMMON_NAME = f"sn{ECC_MAH1H_SERIAL}"
DUMMY_ORG_NAME = "Foobar & Barefoot Inc."
DUMMY_COUNTRY_CODE = "KP"
...
class TestCliCertificate(unittest.TestCase):
    ...
    def _mock_serialport(self):
        ...
        return mock_serialport_patch.start()

    def _mock_kit(self, autodetect_portname="COM-1"):
        """
        Create a mock of the setup_kit() helper

        :param autodetect_portname: Autodetected port name to be returned by mock
        :type autodetect_portname: str
        """
        ...
        # Inject dummy values
        mock_setup_kit.return_value = {"port":autodetect_portname,
                                       "protocol_baud":9600,
    ...
    @patch("pytrustplatform.cli_certificate_main.build_device_csr", autospec=True)
    def test_build_device_csr(self, mock_build_device_csr):
        """
        Test create-device csr action normal case, no errors

        Only testing the CLI part, the device csr building is mocked out
        """
        port = "AUTOCOM"
        self._mock_kit(port)
        mock_serialport = self._mock_serialport()

        testargs = ["pytrust", "certificate", "create-csr", "-o", CSR_OUTPUT_FOLDER,
                    f"--org={DUMMY_ORG_NAME}", f"--country={DUMMY_COUNTRY_CODE}"]
        with patch.object(sys, 'argv', testargs):
            retval = main()

        self.assertEqual(retval, CLI_SUCCESS)
        mock_build_device_csr.assert_called_with(
            ANY,
            csr_filename=DEVICE_CSR_FILENAME,
            common_name=CSR_DUMMY_COMMON_NAME,
            org_name=DUMMY_ORG_NAME,
            country=DUMMY_COUNTRY_CODE,
            force=False)
        mock_serialport.assert_called_with(port, 9600, timeout=10, stopbits=2)

    @patch("pytrustplatform.cli_certificate_main.build_device_csr", autospec=True)
    def test_build_device_csr_port_argument_overrides_autodetect(self, mock_build_device_csr):
        """
        Test create-device csr action normal case, user provided port name
        ...
        testargs = ["pytrust", "-P", myport, "certificate", "create-csr", "-o", CSR_OUTPUT_FOLDER,
                    f"--org={DUMMY_ORG_NAME}", f"--country={DUMMY_COUNTRY_CODE}"]
        with patch.object(sys, 'argv', testargs):
            retval = main()
        ...
        mock_build_device_csr.assert_called_with(
            ...
            country=DUMMY_COUNTRY_CODE,
            force=False)
        mock_serialport.assert_called_with(myport, 9600, timeout=10, stopbits=2)

    @patch("pytrustplatform.cli_certificate_main.build_device_csr", autospec=True)
    def test_build_device_csr_bad_cc(self, mock_build_device_csr):
        """
        Test create-device csr action with error causing premature exit
        ...
        testargs = ["pytrust", "certificate", "create-csr", "-o", CSR_OUTPUT_FOLDER,
                    f"--org={DUMMY_ORG_NAME}", f"--country=BAD_COUNTRY_CODE"]

        # The invalid country code is caught during argument parsing, and argparser
        # will exit directly (actual exit code 2), need to catch SystemExit here to test this.
        # Reference:
